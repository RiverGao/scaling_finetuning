...
with open('../../golden_attention/amr/amr_matrices.pkl', 'rb') as f:
    amr_data = pickle.load(f)  # n_sentence, (word, word)-array

sentence_lengths = []  # store the length of each sentence
r2_scores = []  # R^2 of LR for each layer
n_sentences = len(amr_data)
flatten_amr_data = []  # n_sentences * n_words * (n_words + 1) / 2
...
for layer in range(n_layers):
    ...
    for sentj in range(n_sentences):
        # flatten amr data, only do it once
        if layer == 0:
            sentence_data = amr_data[sentj]  # (word, word)-array
            ...
            flatten_amr_data.extend(flat_sentence_data)
    ...
    y = np.array(flatten_amr_data)
    reg_layer = LinearRegression().fit(X, y)
    score = reg_layer.score(X, y)
    r2_scores.append(score)

np.save(f'../../results/lr_scores/heads_vs_amr/{file_prefix}{model_name}_{model_size}.npy', np.array(r2_scores))
