...
for layer in range(n_layers):
    if not part:
        df_model = pd.read_excel(
            f'../../results/correlation/{model_type}/{model_size}/{head_pool}_{file_prefix}{task}_{method}.xlsx',
            sheet_name=f'layer {layer}')
        df_random = pd.read_excel(
            f'../../results/correlation/{model_type}/{model_size}-random/{head_pool}_{file_prefix}{task}-random_{method}.xlsx',
            sheet_name=f'layer {layer}')
    else:
        df_model = pd.read_excel(
            f'correlation/{model_type}/{model_size}/{part}/{task}_{method}.xlsx.xlsx',
            sheet_name=f'layer {layer}')
        df_random = pd.read_excel(
            f'correlation/{model_type}/{model_size}-random/{part}/{task}-random_{method}.xlsx.xlsx',
            sheet_name=f'layer {layer}')
    ...
    r_amr_random = df_random[f'r {task}'].dropna().to_numpy()
...
rects2 = ax.bar(x + 0.5 * width, r_amr_random_means, width, yerr=r_amr_random_error,
                label=f'{task.capitalize()} Label vs. random', color='xkcd:reddish', ecolor='tab:gray')
...
if not part:
    ...
    ax.set_title(f"{task.capitalize()} Label vs. {model_type.capitalize()} {part.capitalize()} Attention ({model_size})")
