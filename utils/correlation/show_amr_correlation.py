...
def operate(model_size):
    # plot the AMR correlation with llama, alpaca and vicuna within a same model size
    ...
    for layer in range(n_layer):
        ...
        for model in ['llama', 'alpaca-lora', 'vicuna']:
            df_corr_layer = pd.read_excel(
                f'../../results/correlation/{model}/{model_size}/{file_prefix}amr_pearson.xlsx',
                sheet_name=f'layer {layer}')
            # print(df_corr_layer.columns)  # ['subject', 'r number', 'r duration']
            corrs.append(df_corr_layer[f'r AMR'].to_numpy())
    ...
    print(f'{model_size}, {residue}, Max AMR Corr')
    ...
    plot(
        ...
        corr_mean,
        corr_error,
        f'{"Residue " if residue else ""}AMR Correlation of {model_size} Models ({head_pool.capitalize()} Heads)',
        f'{file_prefix}label-amr-{model_size}'
    )
...
def plot(n_ticks, line_labels, means, errors, title, filename):
    ...
    # plt.show()
    fig.savefig(f'../../results/figs/amr/correlation/{filename}.png', dpi=80)
    plt.close(fig)
